import {reconcileThemeFiles} from './theme-reconciliation.js'
import {REMOTE_STRATEGY, LOCAL_STRATEGY} from '../asset-file-syncer.js'
...
describe('reconcileThemeFiles', () => {
  ...
  describe('files only present on remote developer theme', () => {
    test('should download assets from remote theme when `remote` source is selected', async () => {
      ...
      await reconcileThemeFiles(developmentTheme, adminSession, remoteChecksums, defaultThemeFileSystem)
  ...
  describe('files only present on local developer theme', () => {
    test('should delete files from local disk when `remote` source is selected', async () => {
      ...
      // When
      await reconcileThemeFiles(developmentTheme, adminSession, remoteChecksums, localThemeFileSystem)
